...
import 'package:k_tic_tac_toe/game/game_computer.dart';
...
class _GameBoardState extends State<GameBoard> {
  ...
  void initState() {
    ...
    widget.game.onMove = () {
      ...
      if (widget.game.isWin() || widget.game.isDraw()) {
        ...
        if (widget.gamePlayerType == GamePlayerType.onePlayer && widget.game.playerToPlay == TurnOf.player2) {
      ...
      child: Stack(
        children: [
          ...
          GridView.builder(
            ...
            shrinkWrap: true,
            itemCount: widget.game.gameBoardType.size * widget.game.gameBoardType.size,
  ...
  Widget _buildMove(int i, int row, int column) {
    switch (i) {
      case -1:
        if (powerType == PowerType.removeMove && widget.game.playerToPlay == TurnOf.player2) {
          return InkWell(
            onTap: () {
              setState(() {
      ...
      case 1:
        if (powerType == PowerType.removeMove && widget.game.playerToPlay == TurnOf.player1) {
          return InkWell(
            onTap: () {
              setState(() {
                widget.game.gameBoard[row][column] = 0;
...
class BoardPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 1; i < gameBoardType.size; i++) {
      canvas.drawLine(
          Offset(0, baseStrokeDistance * i), Offset(shortestSize * firstStrokeProgress, baseStrokeDistance * i), paint);
    }

    if (secondStrokeProgress > 0) {
      for (int i = 1; i < gameBoardType.size; i++) {
        canvas.drawLine(Offset(baseStrokeDistance * i, 0),
            Offset(baseStrokeDistance * i, shortestSize * secondStrokeProgress), paint);
      }
    }
  }
...
class WinPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    var startPosition = Offset(blockSize * start[1], blockSize * start[0]) + Offset(blockSize / 2, blockSize / 2);
    var endPosition = Offset(blockSize * end[1], blockSize * end[0]) + Offset(blockSize / 2, blockSize / 2);
