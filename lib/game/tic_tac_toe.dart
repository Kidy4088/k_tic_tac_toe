...
class TicTacToe {
  ...
  TicTacToe({
    ...
    this.onMove,
    this.onReset,
  }) : gameBoard = List.generate(gameBoardType.size, (index) => List.generate(gameBoardType.size, (index) => 0));
  ...
  List<List<int>>? _checkThreeMove(int findValue, int winSize, int row, int column) {
    for (int i = -1; i < 2; i++) {
      for (int j = -1; j < 2; j++) {
        /// Avoid the same square
        if (i == 0 && j == 0) {
  ...
  int get moves => gameBoard.fold(0, (previousValue, element) => previousValue + element.where((e) => e != 0).length);
