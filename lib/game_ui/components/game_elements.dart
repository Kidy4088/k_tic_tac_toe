    ...
    return Container(
      child: TweenAnimationBuilder<double>(
          ...
          builder: (context, val, wid) {
            return CustomPaint(
              painter: XPainter(progress: val, color: widget.elementColor, thickness: widget.thickness),
              child: Container(),
            );
          }),
    );
...
class XPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawLine(Offset(0, 0), Offset(size.width * firstStrokeProgress, size.height * firstStrokeProgress), paint);

    if (secondStrokeProgress > 0) {
      canvas.drawLine(Offset(0, size.height),
          Offset(size.width * secondStrokeProgress, size.height * (1.0 - secondStrokeProgress)), paint);
    }
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      child: TweenAnimationBuilder<double>(
          ...
          builder: (context, val, wid) {
            return CustomPaint(
              painter: OPainter(progress: val, color: widget.elementColor, thickness: widget.thickness),
              child: Container(),
            );
          }),
    );
...
class OPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawArc(Rect.fromCircle(center: Offset(size.width / 2, size.height / 2), radius: size.width / 2), 0.0,
        2 * pi * progress, true, paint);
  }
